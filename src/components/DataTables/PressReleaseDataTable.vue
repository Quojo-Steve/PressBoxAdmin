        ...
        <div class="card">
          ...
          <div class="table-responsive">
            <table class="table table-flush" id="datatable-search">
              ...
              <tbody>
                <tr v-for="(release, index) in pressReleases" :key="release.id">
                  <td class="text-sm font-weight-normal">{{ release.title }}</td>
                  <td class="text-sm font-weight-normal">{{ release.published_on }}</td>
                  ...
                  <td class="text-sm font-weight-normal">
                    <i class="fas fa-edit text-primary mr-2" @click="onEdit(release)"></i>
                    <i class="fas fa-trash-alt text-danger" @click="onDelete(release, index)"></i>
                  </td>
                </tr>
              </tbody>
            </table>
...
export default {
  setup() {
    ...
    const onDelete = async (release, index) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            const { data } = await Axios.delete(`/press-releases/${release.id}`);
            if (data.status) {
    ...
    const onEdit = release => {
      router.push(`/press-releases/${release.id}`);
    };
